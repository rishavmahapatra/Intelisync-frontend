...
export default function BlogImageSection({ blogs, categories, selectedCategory }) {
    ...
    <ul className="space-y-4 text-gray-700 flex flex-col items-left"> 
        {categories.map((category, index) => (
            ...
            {/* Right Content Area */}
            <div  className="w-2/3 px-20 mx-auto"> 
                {filteredBlogs.length > 0 ? (
                    filteredBlogs.map((blog) => (
                        <div key={blog.id} className="flex items-start space-x-6 mb-6">
                            <Image 
                                src={blog.image} 
                                alt={blog.title} 
                                width={240} 
                            ...
                            <div className="w-[475px] ml-4"> 
                              <p className="text-gray-500 text-sm leading-tight">
                                ...
                                <p className="text-gray-500 text-sm mt-8 leading-tight">
                                    {blog.description}
                                </p>
                            </div>
                        </div>
